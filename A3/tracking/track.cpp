...
{
	if(argc !=3){
		cout << "Usage: A3 <video file name> <# frames to train on>\n";
		return -1;
	}
	...
	for(int frame =0; frame < trainFrames; frame++){
	...
	imshow("ASEF",asefImg);
	waitKey();
	for(int frame = trainFrames; frame < frameCount; frame++){
		bool success = cap.read(image); // Get next frame of video
		if (!success) {
	    	cout << "Cannot read frame\n";
            break;
		...
		Mat grayImg; // 1280 x 720 image
 		...
 		Mat truncatedImg (grayImg, Rect(384, 104, 512, 512)); // Truncate image to 512 x 512
 		findEdgeMag(truncatedImg);
 		Mat F = truncatedImg.clone();
 		getDFT(F);
 		Mat G;
 		mulSpectrums(F, H, G, DFT_COMPLEX_OUTPUT);
